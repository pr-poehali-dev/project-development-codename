...
def handler(event: dict, context) -> dict:
    ...
    if method == 'POST' and action == 'admin_delete_order':
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(f"DELETE FROM {SCHEMA}.reviews WHERE order_id=%s", (order_id,))
        cur.execute(f"DELETE FROM {SCHEMA}.master_transactions WHERE order_id IN (SELECT id FROM {SCHEMA}.responses WHERE order_id=%s)", (order_id,))
        cur.execute(f"UPDATE {SCHEMA}.orders SET accepted_response_id=NULL WHERE id=%s", (order_id,))
    ...
    if method == 'POST' and action == 'admin_delete_response':
        ...
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(f"DELETE FROM {SCHEMA}.master_transactions WHERE order_id=%s", (response_id,))
        cur.execute(f"UPDATE {SCHEMA}.orders SET accepted_response_id=NULL WHERE accepted_response_id=%s", (response_id,))
    ...
    if method == 'POST' and action == 'admin_delete_master':
        ...
        cur.execute(f"UPDATE {SCHEMA}.orders SET accepted_response_id=NULL WHERE accepted_response_id IN (SELECT id FROM {SCHEMA}.responses WHERE master_id=%s)", (master_id,))
    ...
    if method == 'POST' and action == 'admin_delete_customer':
        ...
        if order_ids:
            ...
            cur.execute(f"DELETE FROM {SCHEMA}.master_transactions WHERE order_id IN (SELECT id FROM {SCHEMA}.responses WHERE order_id IN ({ids_str}))")
            cur.execute(f"UPDATE {SCHEMA}.orders SET accepted_response_id=NULL WHERE id IN ({ids_str})")
